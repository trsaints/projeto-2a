	...
	)
	{
		...
		if (!string.IsNullOrWhiteSpace(searchText))
		{
			...
			if (matchedEvent != null && matchedTodo != null)
				matchedDate = matchedEvent.Due < matchedTodo.Due
						? matchedEvent.Due
						: matchedTodo.Due;
			...
			if (matchedDate.HasValue)
				referenceDate = new DateTime(
					matchedDate.Value.Year,
					matchedDate.Value.Month,
					1
				);
		}
		...
		if (!string.IsNullOrWhiteSpace(searchText))
		{
			...
			filteredEvents =
					filteredEvents.Where(e => e.Name != null
					                          && e.Name.ToLower().Contains(normalized)
					);
			filteredTodos =
					filteredTodos.Where(t => t.Name != null
					                         && t.Name.ToLower().Contains(normalized)
					);
		}
		...
		foreach (var ev in filteredEvents)
		{
			...
			else
			{
				...
				while ((occ = scheduler.GetNext()) != null)
				{
					...
					if (occ.Due >= monthStart) { occurrences.Add((occ.Due, occ.Item)); }
				}
			}
		}
