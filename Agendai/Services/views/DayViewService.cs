...
public static class DayViewService
{
	...
	)
	{
		...
		var minDateEvent = filteredEvents.Any()
				? filteredEvents.Min(e => e.Due)
				: DateTime.MaxValue;
		var minDateTodo =
				filteredTodos.Any() ? filteredTodos.Min(t => t.Due) : DateTime.MaxValue;
		...
		dayOccurrences.AddRange(
			filteredEvents.Where(e => e.Repeats == Repeats.None
			                          && e.Due.Date == referenceDate.Date
			)
		);
		dayOccurrences.AddRange(
			filteredTodos.Where(t => t.Repeats == Repeats.None
			                         && t.Due.Date == referenceDate.Date
			)
		);
